...
export default function SubmissionsPage({
  ...
  const loadSubmissions = async () => {
    ...
    // LSG → USG, LCO/USG/GSC/USED/TGP → OSLD, AO → LCO
    // Letter of Appeal: AO → LCO, LSG → USG, LCO/USG/USED/GSC/TGP → OSLD
    if (orgShortName === 'LCO') {
  ...
  const getSubmissionsByType = (type: string) => {
    // Only show Pending submissions in the main submissions list
  ...
  const handleApprove = async () => {
    ...
    if (selectedSubmission.submission_type === 'Letter of Appeal' && selectedSubmission.event_id) {
      ...
      if (!eventError && eventData) {
        ...
        // Calculate deadline from event end date
        const eventEndDate = new Date(eventData.end_date);
        
        // Determine which type of report is being appealed based on activity_title
        const isAccomplishment = selectedSubmission.activity_title.toLowerCase().includes('accomplishment');
        const isLiquidation = selectedSubmission.activity_title.toLowerCase().includes('liquidation');
        
        // Get today's date and add 3 working days from today
        const today = new Date();
        const newDeadline = addWorkingDays(today, 3);
        const newDeadlineString = newDeadline.toISOString().split('T')[0];

        // Update only the deadline for the type of report being appealed
        const updateData: any = {};
        if (isAccomplishment) {
          updateData.accomplishment_deadline_override = newDeadlineString;
        } else if (isLiquidation) {
          updateData.liquidation_deadline_override = newDeadlineString;
        }

        // Update the event with the new deadline override
        await supabase
          .from('osld_events')
          .update(updateData)
          .eq('id', selectedSubmission.event_id);

        // Create notification for approved appeal with extended deadline
      ...
      // Regular approval flow for non-appeal submissions
      const orgFullNames: Record<string, string> = {
        "OSLD": "Office of Student Leadership and Development",
        "AO": "Accredited Organizations",
        "LSG": "Local Student Government",
  ...
  const renderContent = () => (
    <>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Review and manage all submitted requests</p>
      </div>
      ...
      {!hideNavButtons && (
      <Tabs defaultValue="Request to Conduct Activity" className="w-full">
      ...
      {/* Direct Liquidation Report View for COA - Table View */}
      {hideNavButtons && (
        <div className="space-y-4">
